...
function getName(person) {
  var nameReturn = "Hello, my name is " + person.name;
  return nameReturn;
}
 ...
 */
function makeSmartPerson(tempName) {
  var person = {
    name: tempName,
...
function getLastCarInfo(inventory) {
  ...
  return (
    "This is a " +
    inventory[totalCars - 1].car_make +
    " " +
    inventory[totalCars - 1].car_model
  );
}
...
function sortCarInventory(inventory) {
  for (var i = 0; i < inventory.length; ) {
    for (var j = 0; j < inventory.length; ) {
      if (inventory[j].car_model > inventory[i].car_model) {
        var temp = inventory[i].car_model;
